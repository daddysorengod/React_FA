...
import { useStyles } from "./ReportStaticNavScreen.styles";
...
const ReportStaticNavScreenComponent = forwardRef(
  (props: Props, ref): JSX.Element => {
    ...
    const handleExportExcelFile = () => {
      try {
        if (!globalFundData?.id || !initialValues?.fundNavBatchId) {
          // toast mess here
          console.log("handleExportExcelFile =>> stop");
          return;
        }
        dispatch(
          handleGetExcelFile({
            queryString: getQueryURL(
              "fund-nav-report-api/export-to-xlsx-file",
              {
                fundId: globalFundData?.id,
                fundNavBatchId: initialValues?.fundNavBatchId,
              },
            ),
            fileName: "Nav_History_Report",
          }),
        );
      } catch (error) {
        console.log("handleExportExcelFile =>> fail");
      }
    };
...
const ReportStaticNavScreen = React.memo(ReportStaticNavScreenComponent);
export { ReportStaticNavScreen };
