...
import { useStyles } from "./BaseAccountingEntryDetailsTableEdit.styles";
...
const BaseAccountingEntryDetailsTableEditComponent = forwardRef(
  (props: Props, ref): JSX.Element => {
    ...
    useEffect(() => {
      if (updateData) {
        const newList = updateData.map(item => {
          const newFormatItem = { ...item };
        ...
        tableOptions.listColumn.forEach(columnOption => {
          if (
            !columnOption.hasSummary ||
            columnOption.type !== CO_TYPE.NUMBER
          ) {
            return "";
          }
          const sumReducer = (accumulator: number, currentValue: any) => {
          ...
          const totalSum = updateData.reduce(sumReducer, 0);
          obj[columnOption.key] = formatNumberValueToString(totalSum);
        });
        setSumData({ ...obj });
      }
...
const BaseAccountingEntryDetailsTableEdit = React.memo(
  BaseAccountingEntryDetailsTableEditComponent,
);
export { BaseAccountingEntryDetailsTableEdit };
