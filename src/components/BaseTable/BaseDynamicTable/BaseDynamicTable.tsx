...
import { useStyles } from "@/src/components/BaseTable/BaseDynamicTable/BaseDynamicTable.styles";
...
import { useConfigurationStorage } from "@/src/helpers/dynamic-table/saveConfiguration";
import { TableColumnStatusFA } from "../TableColumnStatusFA";
import { BaseTableColumnCheckBox } from "../BaseTableColumnCheckBox";
...
const BaseDynamicTableComponent = (props: Props): JSX.Element => {
      ...
      return {
        ...
        Cell: ({ cell }) => {
          switch (item.type) {
            ...
            case CO_TYPE.CHECKBOX: {
              return <BaseTableColumnCheckBox cell={cell} />;
            }
            ...
            case CO_TYPE.MATCHED_STATUS: {
              return <BaseTableColumnChip columnOption={item} cell={cell} />;
            }
            case CO_TYPE.FA_STATUS: {
              return <TableColumnStatusFA columnOption={item} cell={cell} />;
            }
            case CO_TYPE.SB_STATUS: {
              return <TableColumnStatusFA columnOption={item} cell={cell} />;
            }
            case CO_TYPE.DATE: {
  ...
  }, [dataListColumn, tableOptions, sizeScreen, selectOptionsListField]);
  ...
  const callApiRowAction = async (
    ...
    try {
      ...
      const apiUrl = `${publicRuntimeConfig.ORIGIN_URL}/${url}?id=${id}${parentId && parentIdName ? `&${parentIdName}=${parentId}` : ""
        }`;
  ...
  const callApiPopUpRowAction = async (
    ...
    if (dialogCode && dialogConfigs[dialogCode]) {
      ...
      typeof dialogCodeCondition !== undefined &&
      Array.isArray(dialogCodeCondition) &&
      dialogCodeCondition.length > 0
    ) {
      let customDialogCode = "";
      dialogCodeCondition.map(config => {
        if (
          !customDialogCode &&
          config.recordStatus &&
          cell.row.original[config.recordStatus] &&
          cell.row.original[config.recordStatus].toString() ===
          config.configValueEqual
        ) {
          customDialogCode = config.dialogCode ? config.dialogCode : "";
        }
      });
      if (customDialogCode) {
        setCurrentDialogConfig(formConfigFiles[customDialogCode]);
      } else {
        return;
      }
    } else {
  ...
  const callApiActionAll = async (
    ...
    try {
      ...
      const apiUrl = `${publicRuntimeConfig.ORIGIN_URL}/${url}${parentId && parentIdName ? `?${parentIdName}=${parentId}` : ""
        }`;
  ...
  const handleOpenAddNewFormDialog = useCallback(() => {
    ...
    // setTimeout(() => {
    //   console.log("handleOpenAddNewFormDialog =>>>>")
  ...
  useConfigurationStorage(
    { ordering, setOrdering },
    { initialVisibleColumns, setInitialVisibleColumns },
  ...
  return (
    ...
    >
      ...
      <Box
        ...
        sx={{
          height: !!tableOptions.tableTitle ? "calc(100% - 32px)" : "100%",
        }}
      >
        <MaterialReactTable
          columns={columns2}
          ...
          enableTopToolbar={!(tableOptions.enableTopToolbar == false)}
      ...
      <BaseDialogCreate
        ...
        useCurrentRecordId={tableOptions?.useCurrentRecordId}
      />
    </Box>
  );
};
const BaseDynamicTable = React.memo(BaseDynamicTableComponent);
export { BaseDynamicTable };
